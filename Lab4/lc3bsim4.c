...
int16_t ALU_Val, SHF_Val, MDR_Val, PC_Val, MAR_Val, MEM_Val, BEN_Val, TEMP_Val, VECTOR_Val, USP_Val, SSP_Val, SP_Val, PSR_Val, PB_Val, PREV_PSR; // Temp register values for each instruction to reference
   /* 
...
void drive_bus() {
   ...
   */
    ...
    /****************GATE_USP****************/
    if(GetGATE_USP(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]) && (CURRENT_LATCHES.PSR & 0x8000)){
        BUS = USP_Val;
    }
    /****************GATE_SSP****************/
    if(GetGATE_SSP(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]) && ((CURRENT_LATCHES.PSR & 0x8000) == 0)){
   ...
   */       
    ...
    */
    ...
    if(GetLD_EX(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER])){
        uint16_t PR = ((NEXT_LATCHES.MAR >= 0x0000) && (NEXT_LATCHES.MAR < 0x3000) && ((CURRENT_LATCHES.PSR >> 15) & 0x01)) ? 1 : 0;
        PR = PR << 2;
        int16_t UA = (NEXT_LATCHES.MAR & 0x0001) && (GetDATA_SIZE(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]) == 1);
        UA = UA << 1;
        int16_t UO = (GetUNKOWN(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]));
        NEXT_LATCHES.EXCV = PR | UA | UO;
        // printf("Exception Vector: 0x%0.4x\n", NEXT_LATCHES.EXCV);
    ...
    if(GetLD_VECTOR(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER])){
        if(GetVECTORMUX(CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER]) == 1){
            ...
            if (CURRENT_LATCHES.EXCV & 0b100) { // Check if bit 2 is set
                VECTOR_Val = 0x02;
                // printf("Protected Access\n");
            } 
            else if (CURRENT_LATCHES.EXCV & 0b010) { // Check if bit 1 is set
                VECTOR_Val = 0x03;
                // printf("Unaligned Access\n");
            } 
            else if (CURRENT_LATCHES.EXCV & 0b001) { // Check if bit 0 is set
                VECTOR_Val = 0x04;
